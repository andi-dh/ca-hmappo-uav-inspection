...
def evaluate(args: argparse.Namespace) -> None:
    ...
    for seed in range(args.base_seed, args.base_seed + args.num_seeds):
        ...
        env = CapabilityAwareMAPPOEnvWrapper(scenario=args.scenario, seed=seed)
        ...
        metrics["seed"] = seed
        metrics["baseline"] = "capability_aware_mappo"
    ...
    summary_file = output_dir / f"scenario_{args.scenario}_capability_aware_mappo_{mode}_summary.csv"
    mean_std_file = output_dir / f"scenario_{args.scenario}_capability_aware_mappo_{mode}_mean_std.csv"
    mean_std = _compute_mean_std(rows)
    _write_rows_csv(rows, summary_file)
    _write_rows_csv([mean_std], mean_std_file)
    print("Capability-aware MAPPO evaluation completed")
...
def _new_diagnostics() -> dict:
    return {
        ...
        "distance_sum": 0.0,
        "distance_count": 0,
        "time_in_candidate_range": 0,
    }


def _update_diagnostics(env: CapabilityAwareMAPPOEnvWrapper, actions, diagnostics: dict) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate Capability-Aware Heterogeneous MAPPO.")
    parser.add_argument("--scenario", type=int, default=0, choices=[0, 1])
    parser.add_argument("--model", type=str, default="models/3c_capability_aware_mappo/capability_aware_mappo_scenario_0.pt")
