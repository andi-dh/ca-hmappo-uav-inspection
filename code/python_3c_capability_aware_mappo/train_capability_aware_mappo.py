...
from capability_aware_env_wrapper import CapabilityAwareMAPPOEnvWrapper
...
def train(args: argparse.Namespace) -> None:
    ...
    for update in range(1, args.num_updates + 1):
        ...
        for _ in range(args.rollout_steps):
            ...
            buffer.add(obs, state, actions_t.cpu().numpy(), log_probs_t.cpu().numpy(), rewards, dones, float(value_t.item()), masks)
        ...
        if update % args.log_interval == 0 or update == 1:
            print(
                ...
                f"policy_loss={row['policy_loss']:.3f} value_loss={row['value_loss']:.3f} entropy={row['entropy']:.3f}"
            )
        if args.checkpoint_interval > 0 and update % args.checkpoint_interval == 0:
            checkpoint_file = model_dir / f"capability_aware_mappo_scenario_{args.scenario}_update_{update}.pt"
            _save_model(actor, critic, env, args.scenario, checkpoint_file)

    curve_file = output_dir / f"training_curve_capability_aware_mappo_scenario_{args.scenario}.csv"
    _write_rows_csv(curve_rows, curve_file)
    _save_training_curve(curve_rows, output_dir / f"training_curve_capability_aware_mappo_scenario_{args.scenario}.png")
    model_file = model_dir / f"capability_aware_mappo_scenario_{args.scenario}.pt"
    _save_model(actor, critic, env, args.scenario, model_file)
    print(f"Saved training curve: {curve_file}")
    print(f"Saved model: {model_file}")


def _save_model(actor: HeterogeneousActor, critic: Critic, env: CapabilityAwareMAPPOEnvWrapper, scenario: int, path: Path) -> None:
    torch.save(
        {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Capability-Aware Heterogeneous MAPPO.")
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/3c_capability_aware_mappo")
    parser.add_argument("--model-dir", type=str, default="models/3c_capability_aware_mappo")
