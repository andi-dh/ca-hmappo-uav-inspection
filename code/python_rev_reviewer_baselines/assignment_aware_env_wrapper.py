...
for folder in ["python_baseline", "python_3a_homogeneous_mappo", "python_3c_capability_aware_mappo"]:
    path = ROOT / folder
...
class ScalableHeterogeneousUAVEnv(HeterogeneousUAVEnv):
    """Reviewer-only environment variant with multiple fixed-wing scouts."""
...
class AssignmentAwareMAPPOEnvWrapper(CapabilityAwareMAPPOEnvWrapper):
    ...
    too early. This reviewer-facing variant makes the candidate coordination layer
