...
THREE_A_DIR = Path(__file__).resolve().parents[1] / "python_3a_homogeneous_mappo"
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    for seed in range(args.base_seed, args.base_seed + args.num_seeds):
        ...
        metrics["seed"] = seed
        metrics["baseline"] = "heterogeneous_mappo"
    ...
    summary_file = output_dir / f"scenario_{args.scenario}_heterogeneous_mappo_{mode}_summary.csv"
    mean_std_file = output_dir / f"scenario_{args.scenario}_heterogeneous_mappo_{mode}_mean_std.csv"
    mean_std = _compute_mean_std(rows)
    _write_rows_csv(rows, summary_file)
    _write_rows_csv([mean_std], mean_std_file)
    print("Heterogeneous MAPPO evaluation completed")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate Heterogeneous MAPPO baseline.")
    parser.add_argument("--scenario", type=int, default=0, choices=[0, 1])
    parser.add_argument("--model", type=str, default="models/3b_heterogeneous_mappo/heterogeneous_mappo_scenario_0.pt")
    parser.add_argument("--num-seeds", type=int, default=10)
    parser.add_argument("--base-seed", type=int, default=0)
    parser.add_argument("--hidden-dim", type=int, default=128)
    parser.add_argument("--device", type=str, default="cpu")
    parser.add_argument("--output-dir", type=str, default="outputs/3b_heterogeneous_mappo")
