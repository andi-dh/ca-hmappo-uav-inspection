...
def compare(args: argparse.Namespace) -> None:
    ...
    rows.append(
        _read_mean_std(
            Path(args.three_a_dir) / f"scenario_{args.scenario}_homogeneous_mappo_{args.eval_mode}_mean_std.csv",
            f"homogeneous_mappo_{args.eval_mode}",
            args.scenario,
        )
    )
    rows.append(
        _read_mean_std(
            Path(args.three_b_dir) / f"scenario_{args.scenario}_heterogeneous_mappo_{args.eval_mode}_mean_std.csv",
            f"heterogeneous_mappo_{args.eval_mode}",
            args.scenario,
        )
    )
    rows = [row for row in rows if row]
    output_file = Path(args.three_b_dir) / f"scenario_{args.scenario}_3b_baseline_comparison_{args.eval_mode}.csv"
    output_file.parent.mkdir(parents=True, exist_ok=True)
    with output_file.open("w", newline="", encoding="utf-8") as file:
        writer = csv.DictWriter(file, fieldnames=["baseline", "scenario"] + KEY_METRICS)
        writer.writeheader()
        writer.writerows(rows)
    print("3B baseline comparison saved")
    print(output_file)
    for row in rows:
        print(row)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare 3B baselines.")
